--QUESTIONS ASKED IN THE EPAM INTERVIEW
SELECT o1.orderid + 1 ,O1.OrderID-- missing_after
FROM SALES.Orders o1
LEFT JOIN Sales.Orders o2 
    ON o2.orderid = o1.orderid + 1
...
SELECT 
   CUSTOMER_SUMMARY.CUSTOMERID,
   COALESCE(AVG(DAYS_BETWEEN_ORDERS),99999) AVERAGE_DAYS_BETWEEN_ORDERS,
   RANK() OVER(ORDER BY  COALESCE(AVG(DAYS_BETWEEN_ORDERS),99999)) CUSTOMER_LOYALTY
FROM CUSTOMER_SUMMARY
GROUP BY CUSTOMER_SUMMARY. CustomerID
...
-- do this with the cte

WITH SALES_SUMMARY AS
(
SELECT
   MONTH(OrderDate) SALES_MONTH,
   SUM(Sales) TOTAL_SALES,
   SUM(Sales) CURRENT_SALES,
   LAG(SUM(Sales),1) OVER(ORDER BY MONTH(ORDERDATE)) PREVIOUS_SALES
FROM Sales.Orders
GROUP BY MONTH(OrderDate)
)
SELECT *,
    (CURRENT_SALES - PREVIOUS_SALES) MOM,
   ROUND( CAST((CURRENT_SALES - PREVIOUS_SALES)AS float) / PREVIOUS_SALES * 100,3) MOM_PERCENTAGE
FROM SALES_SUMMARY;
...
FROM ( 
    SELECT 
        ...
        LAG(SUM(Sales),1) OVER(ORDER BY MONTH(ORDERDATE)) [PREVIOUS SALES]
    FROM Sales.Orders
    GROUP BY MONTH(OrderDate)
)T
