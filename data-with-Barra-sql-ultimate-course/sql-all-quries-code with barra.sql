...
SELECT 
	...
	OrderID,
	ProductID,
	FIRST_VALUE(Sales) OVER (PARTITION BY PRODUCTID ORDER BY SALES ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW),
	FIRST_VALUE(Sales) OVER (PARTITION BY PRODUCTID ORDER BY SALES DESC ) AS [MAX SALES FOR EACH PRODUCT],
	LAST_VALUE(SALES) OVER (PARTITION BY PRODUCTID ORDER BY SALES ASC ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) AS [MIN SALES FOR EACH PRODUCT]
FROM Sales.Orders
